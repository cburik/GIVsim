...
output=sprintf('%s_rho_%1.1f_rhoe_%1.1f_delta_%i.RData',name,rho
    ,rho_e,delta)
...
for(i_rep in 1:rep){
    ...
    y =  markers %*% betas[,1]  * m_weight + coef*T + errors[,1] * e_weight
    ...
    y_R = T_R*coef + markers_R %*% betas[,1] * m_weight + errors_R[,1]* e_weight
